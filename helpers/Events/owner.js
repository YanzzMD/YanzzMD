...
export default async function on({ cht, Exp, store, ev, is }) {
   ...
   const { id } = cht
    ...
    function sendPremInfo({ _text, text }, cust=false, number){
        return Exp.sendMessage(number || id, {
            text:`${_text ? (_text + "\n\n" + text) : text}`,
                contextInfo: {
                    externalAdReply: {
                    title: !cust ? "🔐Premium Access Only!" : "🔓Unlocked Premium Access Only!",
                    body: !cust ?  "Dapatin akses premium buat buka fitur² terkunci" : "Sekarang kamu adalah user 🔑Premium, dapat menggunakan fitur² terkunci!",
    ...
    ev.on({ 
        ...
        let txt = `*Berhasil men${cht.cmd == "addenergy" ? "ambahkan":"gurangi"} ${num} ⚡Limit ke @${sender}*`
            txt += "\n\n*[🔋] Limit*"
            txt += "\n⚡Limit: " + user.energy
            txt += `\n\n- Status: ${user.charging ? "🟢Charging" : " ⚫Discharging"}`
            txt += "\n- Charging Speed: ⚡" + rate + " Limit/" + speed
            txt += "\n- Max Charge: " + max
