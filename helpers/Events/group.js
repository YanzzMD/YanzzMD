...
export default async function on({ cht, Exp, store, ev, is }) {
   const { id } = cht
    ev.on({ 
        ...
        if(!is.group) return cht.reply("Khusus group yaaa!")
        if(!is.groupAdmins) return cht.reply("loh kamu kan bukan admin!")
        if(!is.botAdmin) return cht.reply("adminin aku donggg :(")
    ...
    ev.on({ 
        ...
        if(!is.group) return cht.reply("Khusus group yaaa!")
        if(!is.groupAdmins) return cht.reply("loh kamu kan bukan admin!")
        if(!is.botAdmin) return cht.reply("adminin aku doonggg :(")
        if(cht.mention.length > 0){
            if(is.botMention && cht.cmd == "kick") return msg.reply("AK GMW KELUAR!!")
            let { status } = (await Exp.groupParticipantsUpdate(id, cht.mention, cht.cmd == "kick"? "remove" : "add"))[0]
            if(status == 408) return cht.reply('Dia baru aja keluar dari grub ini!')
            if(status == 409) return cht.reply('Dia udah join bang!')
            if(status == 500) return cht.reply('Grubnya penuh!')
            if(status == 403) return cht.reply("Maaf, gabisa ditambah karna private acc")
        } else {
            let reply = `*masukin nomor/Reply/tag orangnya yang mau ${cht.cmd} dari group!*\n\nExample: \n\n*Caranya ginii #1* => _Dengan reply pesan target_\n - ${prefix + cht.cmd} \n \n*Cara #2* => _Dengan tag target_\n - ${prefix + cht.cmd} @rifza \n \n*Cara #2* => _Dengan nomor target_\n - ${prefix + cht.cmd} +62 831-xxxx-xxxx` 
            cht.reply(reply)
        }
	})
	...
	ev.on({ 
        ...
        if(cht.mention.length > 0){
            ...
            let reply = `*masukin nomor/Reply/tag orangnya yang mau ambil pp nya!*\n\nExample: \n\n*Caranya ginii #1* => _Dengan reply pesan target_\n - ${prefix + cht.cmd} \n \n*Cara #2* => _Dengan tag target_\n - ${prefix + cht.cmd} @rifza \n \n*Cara #2* => _Dengan nomor target_\n - ${prefix + cht.cmd} +62 831-xxxx-xxxx` 
            cht.reply(reply)
        }
	})
