...
export default async function on({ cht, Exp, store, ev, is }) {
    ...
    ev.on({ 
      ...
      tag: 'downloader',
      badword: true,
      args: "Masukin url/judul videonya dong, aku kan bingung kalo gada judulnya!",
      energy: 5
    }, async ({ args, urls }) => {
        const _key = keys[sender]
        let q = urls || args || null
        if (!q) return cht.reply('Msukin url/judul videonya dong, aku kan bingung kalo gada judulnya!')
        try {
            ...
            let audio = {
                ...
                contextInfo: {
                    externalAdReply: {
                        ...
                        sourceUrl: item.url,
                        mediaUrl: "http://ẉa.me/6282245186794?text=Idmsg: " + Math.floor(Math.random() * 100000000000000000),
